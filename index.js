...
var lengthOfLongestSubstring = function(s) {
    var sub = s.substr(0,1);
    var ls ;
    s.length > 0 ? ls = 1 : ls = 0;  
    var l = 0;
    let pervSub ='';
    for(var i = 0 ; i < s.length ; i++)
    {
        if(!sub.includes(s[i+1]))
        {
            sub = s.substring(l,i+2);
            for(var n = pervSub.length -1 ; n >= 0 ; n--)
            {
                if(!sub.includes(pervSub[n]))
                {
                    sub = s.substring(n , i+2);
                }
                else{
                    break;
                }
            } 
            sub.length > ls ? ls = sub.length : ls = ls;
            console.log(sub);
        }
        else{
            l=i+1;
            pervSub = s.substring(0,l);
            for(var n = pervSub.length -1 ; n >= 0 ; n--)
            {
                if(!sub.includes(pervSub[n]))
                {
